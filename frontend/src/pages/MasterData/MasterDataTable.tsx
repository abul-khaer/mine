...
export default function MasterDataTable({ category, label }: Props) {
  ...
  const { data: items = [], isLoading } = useQuery<MasterDataItem[]>({
    ...
    queryFn: () => api.get(`/master-data/admin?category=${category}`).then((r) => r.data),
  });

  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ['master-data', category] });
    qc.invalidateQueries({ queryKey: ['master-data-select', category] });
  };

  const createMutation = useMutation({
    mutationFn: (data: { category: string; name: string; description?: string }) =>
      api.post('/master-data', data),
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, is_active }: { id: number; is_active: boolean }) =>
      api.put(`/master-data/${id}`, { is_active }),
    onSuccess: () => invalidate(),
    onError: () => toast.error('Gagal mengubah status'),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.delete(`/master-data/${id}`),
    onSuccess: () => {
  ...
  return (
    <div className="card">
      ...
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-base font-bold text-forest-deep">{label}</h2>
        <button
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-cream-100">
            {isLoading ? (
              <tr>
                <td colSpan={4} className="text-center py-8 text-forest-mid/40">
                  Memuat...
                </td>
              </tr>
            ) : items.length === 0 ? (
                ...
                <tr key={item.id} className="hover:bg-cream-100/60 transition-colors">
                  ...
                  <td className="table-cell text-forest-mid">
                    {editId === item.id ? (
                      ...
                      item.description || (
                        <span className="text-forest-mid/30">—</span>
                      )
                    )}
                  </td>
      ...
      <ConfirmDialog
        ...
        message={`Hapus item ini dari kategori ${label}?`}
